...
INSERT INTO `settings` VALUES (1,5,1,1),(2,5,5,2);
...
INSERT INTO `users` VALUES (1,'crentagon','crentagon@gmail.com','f5a287da5aa52f63d24abda025b5e49d');
...
-- Dump completed on 2014-04-25 16:26:29

-- New tables!!1
CREATE TABLE `productivore_db`.`menus` (
  `menu_id` INT NOT NULL AUTO_INCREMENT,
  `menu_name` VARCHAR(64) NOT NULL,
  `appling_id` INT NULL,
  PRIMARY KEY (`menu_id`),
  INDEX `menus_appling_id_fk_idx` (`appling_id` ASC),
  CONSTRAINT `menus_appling_id_fk`
    FOREIGN KEY (`appling_id`)
    REFERENCES `productivore_db`.`applings` (`appling_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
COMMENT = 'Those stuff in the navbar.';

ALTER TABLE `productivore_db`.`menus` 
ADD COLUMN `menu_url` VARCHAR(32) NOT NULL AFTER `menu_name`;

INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Notifications', 'index', '0');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Appling Manager', 'applings', '0');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Control Panel', 'control', '0');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Achievements', 'achievements', '4');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Anticipate', 'anticipate', '4');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('Lessons', 'lessons', '4');
INSERT INTO `productivore_db`.`menus` (`menu_name`, `menu_url`, `appling_id`) VALUES ('My Profile', 'selfdiscovery', '4');

ALTER TABLE `productivore_db`.`menus` 
ADD COLUMN `parent_menu_id` INT NULL AFTER `appling_id`,
ADD INDEX `menus_menu_id_fk_idx` (`parent_menu_id` ASC);
ALTER TABLE `productivore_db`.`menus` 
ADD CONSTRAINT `menus_menu_id_fk`
  FOREIGN KEY (`parent_menu_id`)
  REFERENCES `productivore_db`.`menus` (`menu_id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

  UPDATE `productivore_db`.`applings` SET `appling_url`='site' WHERE `appling_id`='0';
